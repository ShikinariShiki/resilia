...
const dataFile = path.join(__dirname, 'onboarding-data.json');
...
for (const file of locales) {
    ...
    if (content.includes('landing: {')) {
        const injection = `onboarding: ${JSON.stringify(t, null, 8).replace(/\\n/g, '')},\n    landing: {`;
        content = content.replace('landing: {', injection);
        fs.writeFileSync(filePath, content, 'utf8');
        console.log("Patched onboarding data for", file);
    } else {

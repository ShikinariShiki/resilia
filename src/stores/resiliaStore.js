...
import { predictMissionWeights, generatePersonalizedMissions, recommendMissions, adaptWeightsKNN } from '../ml/recommend'
...
export const useResiliaStore = defineStore('resilia', () => {
    ...
    // Personalization questionnaire answers (drive the ML model)
    const userGoal = ref(_savedProfile?.userGoal || '')
    const userStressor = ref(_savedProfile?.userStressor || '')
    const userCoping = ref(_savedProfile?.userCoping || '')
    const userTime = ref(_savedProfile?.userTime || '')
    const userConfidence = ref(_savedProfile?.userConfidence || '')
    ...
    // Regenerate personalized daily missions on a new day for onboarded users
    if (isNewDay && onboarded.value) {
        const _w = userPersonalization.value?.dailyMissionWeights
        if (_w && Object.keys(_w).length) {
            dailyMissions.value = generatePersonalizedMissions(_w)
            saveDailySession()
        }
    }
    ...
    const regionData = ref([
        { country: 'Indonesia', code: 'ID', flag: '/flags/id.svg', population: 275.5, gdpPerCapita: 4788, hdi: 0.705, disasterRiskIndex: 46.92, readiness: 74, responders: 1240, recentDisaster: 'Cianjur Earthquake 2022', annualDisasters: 3092, topDisasters: 'Earthquake, Flood, Volcano', emergencyHotline: '112 / 119' },
        { country: 'Philippines', code: 'PH', flag: '/flags/ph.svg', population: 115.6, gdpPerCapita: 3623, hdi: 0.699, disasterRiskIndex: 45.12, readiness: 68, responders: 890, recentDisaster: 'Typhoon Rai 2021', annualDisasters: 1058, topDisasters: 'Typhoon, Earthquake, Flood', emergencyHotline: '911' },
        { country: 'Vietnam', code: 'VN', flag: '/flags/vn.svg', population: 99.5, gdpPerCapita: 4164, hdi: 0.703, disasterRiskIndex: 24.96, readiness: 55, responders: 420, recentDisaster: 'Typhoon Noru 2022', annualDisasters: 394, topDisasters: 'Typhoon, Flood, Landslide', emergencyHotline: '113 / 115' },
        { country: 'Thailand', code: 'TH', flag: '/flags/th.svg', population: 71.8, gdpPerCapita: 7233, hdi: 0.800, disasterRiskIndex: 14.37, readiness: 82, responders: 610, recentDisaster: 'Southern Floods 2023', annualDisasters: 128, topDisasters: 'Flood, Drought, Landslide', emergencyHotline: '1669' },
        { country: 'Malaysia', code: 'MY', flag: '/flags/my.svg', population: 33.9, gdpPerCapita: 12448, hdi: 0.803, disasterRiskIndex: 8.63, readiness: 78, responders: 340, recentDisaster: 'Selangor Floods 2021', annualDisasters: 52, topDisasters: 'Flood, Landslide, Haze', emergencyHotline: '999' },
        { country: 'Myanmar', code: 'MM', flag: '/flags/mm.svg', population: 54.4, gdpPerCapita: 1210, hdi: 0.585, disasterRiskIndex: 42.55, readiness: 42, responders: 180, recentDisaster: 'Cyclone Mocha 2023', annualDisasters: 312, topDisasters: 'Cyclone, Flood, Earthquake', emergencyHotline: '199' },
        { country: 'Cambodia', code: 'KH', flag: '/flags/kh.svg', population: 16.9, gdpPerCapita: 1768, hdi: 0.593, disasterRiskIndex: 16.58, readiness: 48, responders: 95, recentDisaster: 'Mekong Floods 2022', annualDisasters: 67, topDisasters: 'Flood, Drought, Storm', emergencyHotline: '119' },
        { country: 'Laos', code: 'LA', flag: '/flags/la.svg', population: 7.5, gdpPerCapita: 2054, hdi: 0.607, disasterRiskIndex: 15.57, readiness: 38, responders: 45, recentDisaster: 'Dam Collapse 2018', annualDisasters: 34, topDisasters: 'Flood, Drought, UXO', emergencyHotline: '1195' },
        { country: 'Singapore', code: 'SG', flag: '/flags/sg.svg', population: 5.9, gdpPerCapita: 72794, hdi: 0.939, disasterRiskIndex: 2.42, readiness: 91, responders: 220, recentDisaster: 'Heatwave 2023', annualDisasters: 3, topDisasters: 'Heatwave, Flood, Haze', emergencyHotline: '995' },
        { country: 'Brunei', code: 'BN', flag: '/flags/bn.svg', population: 0.45, gdpPerCapita: 31449, hdi: 0.829, disasterRiskIndex: 3.08, readiness: 85, responders: 42, recentDisaster: 'Floods 2021', annualDisasters: 5, topDisasters: 'Flood, Haze, Fire', emergencyHotline: '993' },
    ])
    ...
    function completeOnboarding(payload, country, age, gender, disasterExp) {
        // Backward compatible: accepts the legacy positional signature
        // (name, country, age, gender, disasterExp) or a single object with the
        // full personalization questionnaire.
        const data = (typeof payload === 'object' && payload !== null)
            ? payload
            : { name: payload, country, age, gender, disasterExp }
        userName.value = data.name
        countryCode.value = data.country
        if (data.age) userAge.value = data.age
        if (data.gender) userGender.value = data.gender
        if (data.disasterExp !== undefined) hasDisasterExperience.value = data.disasterExp
        if (data.goal) userGoal.value = data.goal
        if (data.stressor) userStressor.value = data.stressor
        if (data.coping) userCoping.value = data.coping
        if (data.time) userTime.value = data.time
        if (data.confidence) userConfidence.value = data.confidence
    ...
    function _saveProfile() {
        localStorage.setItem('resilia_profile', JSON.stringify({
            ...
            userStressor: userStressor.value,
            userCoping: userCoping.value,
            userTime: userTime.value,
            userConfidence: userConfidence.value,
            bio: bio.value,
            avatarColor: avatarColor.value,
            avatarUrl: avatarUrl.value,
            joinDate: joinDate.value,
    ...
    // ═══ ML Personalization (trained softmax model) ═══
    // Maps the onboarding questionnaire to a probability distribution over
    // mission categories using the offline-trained model in src/ml, then
    // derives content settings and a personalized daily-mission set from it.
    function computePersonalization() {
        const answers = {
            age: userAge.value,
            disasterExp: hasDisasterExperience.value === true,
            goal: userGoal.value,
            stressor: userStressor.value,
            coping: userCoping.value,
            time: userTime.value,
            confidence: userConfidence.value,
        }

        // Trained model -> weight per mission category (sums to 1).
        const weights = predictMissionWeights(answers)

        const hasExperience = hasDisasterExperience.value === true
        const isAdult = userAge.value === 'adult'
        let narrativeTone = 'supportive'
        if (userConfidence.value === 'high' && hasExperience) narrativeTone = 'challenging'
        else if (isAdult && userConfidence.value !== 'low') narrativeTone = 'neutral'

        const contentDifficulty = Math.max(0, Math.min(1,
            0.45
            + (hasExperience ? 0.15 : -0.05)
            + (userConfidence.value === 'high' ? 0.2 : userConfidence.value === 'low' ? -0.2 : 0)
            + (isAdult ? 0.05 : -0.1)
        ))
        const quizFrequency = Math.max(0, Math.min(1,
            0.4 + ((weights.lesson || 0) + (weights.dashboard || 0)) * 0.8
        ))

        const topicByCat = {
            lesson: 'foundations', rpg: 'crisis-response', checkin: 'self-awareness',
            toolkit: 'coping-skills', donate: 'community', dashboard: 'regional-data',
        }
        const focusTopics = Object.keys(weights)
            .sort((a, b) => weights[b] - weights[a])
            .slice(0, 3)
            .map((c) => topicByCat[c] || c)
        ...
        const personalization = {
            ...
            narrativeTone,
            focusTopics,
            recommendedCountry: countryChapterMap[countryCode.value] || 'ch1',
            dailyMissionWeights: weights,
            model: 'softmax-v1',
        }
    ...
    function updateStability(scorePercent, avgLikert, currentAnswers = []) {
        ...
        const adaptedWeights = adaptWeightsKNN(baseWeights, currentAnswers, avgLikert || ((100 - scorePercent) / 100 * 6 + 1))
    ...
    return {
        ...
        userGoal, userStressor, userCoping, userTime, userConfidence,

<template>
  <div>
    ...
    <div v-motion class="mb-10 sm:mb-12">
      <div class="flex items-center gap-3 mb-3">
        ...
        <div>
          ...
          <p class="text-gray-400 font-body text-sm sm:text-base mt-0.5">{{ t('dashboard.subtitle') }}</p>
        </div>
      </div>
    </div>

    <!-- Live Threat Ticker -->
    <div v-motion class="bg-red-50 dark:bg-red-900/15 rounded-2xl p-4 mb-8 overflow-hidden border border-red-200/30 dark:border-red-800/30" style="animation-delay:0.02s">
      <div class="flex items-center gap-3">
        <span class="text-xs font-heading font-bold text-red-500 uppercase tracking-wider animate-pulse flex-shrink-0"><PhLightning :size="14" weight="fill" class="inline" /> {{ t('dashboard.live') }}</span>
        <div class="overflow-hidden flex-1">
          <div class="flex gap-12 ticker-scroll whitespace-nowrap text-xs font-body text-red-600 dark:text-red-400">
            <span v-for="(alert, i) in tickerAlerts" :key="i" class="inline-flex items-center gap-2">
              <img :src="alert.flag" class="w-6 h-auto inline-block rounded-sm shadow-sm" alt="Flag" />
              {{ alert.text }}
            </span>
          </div>
        </div>
      ...
      <div class="bg-gradient-to-br from-teal-500 to-teal-700 rounded-xl sm:rounded-2xl p-5 sm:p-7 text-white relative overflow-hidden">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[10px] text-teal-200">{{ t('dashboard.readinessSub') }}</p>
          </div>
        </div>
      </div>

      <BaseCard glass v-for="m in topMetrics" :key="m.label" padding="lg">
    ...
    <BaseCard glass padding="lg" v-motion class="mb-8 sm:mb-10" style="animation-delay:0.04s">
      <h3 class="font-heading text-base sm:text-lg font-bold text-ink dark:text-white mb-2 flex items-center gap-2">
        <PhChartBar :size="20" weight="fill" class="inline text-teal-500" /> {{ t('dashboard.riskComparison') }}
      </h3>
      <p class="text-xs text-gray-500 mb-6 font-body">{{ t('dashboard.riskComparisonSub') }}</p>
    ...
    <div v-motion class="mb-8 sm:mb-10" style="animation-delay:0.06s">
      <h3 class="font-heading text-base sm:text-lg font-bold text-ink dark:text-white mb-5 flex items-center gap-2">
        ...
        {{ t('dashboard.intelHub') }}
      </h3>
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        <BaseCard glass padding="sm" v-for="r in store.regionData" :key="r.country" hoverable
          class="country-card relative overflow-hidden ring-teal-500"
          :class="{ 'ring-2': selectedCountry === r.code }"
          ...
          <div class="text-center">
            ...
            <div class="bg-red-50/80 dark:bg-red-900/20 rounded-lg px-2 py-1.5 mt-2">
              <p class="text-[8px] font-heading text-red-400 uppercase tracking-wider">{{ t('dashboard.emergency') }}</p>
    ...
    <Transition name="expand">
      <BaseCard glass padding="lg" v-if="selectedCountry" v-motion class="mb-8 sm:mb-10">
        <div class="flex items-start justify-between gap-4 mb-6">
          <div class="flex items-center gap-4">
            <img :src="selectedData.flag" class="w-6 h-auto inline-block rounded-sm shadow-sm" alt="Flag" />
            <div>
              <h3 class="font-heading text-xl font-bold text-ink dark:text-white">{{ selectedData.country }}</h3>
              <p class="text-xs text-gray-400 font-body">{{ t('dashboard.populationGdp').replace('{pop}', selectedData.population).replace('{gdp}', selectedData.gdpPerCapita?.toLocaleString()) }}</p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div class="bg-gray-50 dark:bg-slate-700/30 rounded-xl p-4 text-center">
            <p class="text-[9px] font-heading text-gray-400 uppercase mb-1">{{ t('dashboard.riskIndex') }}</p>
            <p class="font-heading text-2xl font-bold" :class="riskColor(selectedData.disasterRiskIndex)">{{ selectedData.disasterRiskIndex }}</p>
          </div>
          <div class="bg-gray-50 dark:bg-slate-700/30 rounded-xl p-4 text-center">
            <p class="text-[9px] font-heading text-gray-400 uppercase mb-1">{{ t('dashboard.hdi') }}</p>
            <p class="font-heading text-2xl font-bold text-ink dark:text-white">{{ selectedData.hdi }}</p>
          </div>
          <div class="bg-gray-50 dark:bg-slate-700/30 rounded-xl p-4 text-center">
            <p class="text-[9px] font-heading text-gray-400 uppercase mb-1">{{ t('dashboard.disastersYr') }}</p>
            <p class="font-heading text-2xl font-bold text-amber-500">{{ selectedData.annualDisasters }}</p>
          </div>
          <div class="bg-gray-50 dark:bg-slate-700/30 rounded-xl p-4 text-center">
            <p class="text-[9px] font-heading text-gray-400 uppercase mb-1">{{ t('dashboard.responders') }}</p>
        ...
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="bg-amber-50/50 dark:bg-amber-900/10 rounded-xl p-4">
            <p class="text-[9px] font-heading text-amber-600 uppercase mb-1">{{ t('dashboard.topThreats') }}</p>
          ...
          <div class="bg-red-50/50 dark:bg-red-900/10 rounded-xl p-4">
            <p class="text-[9px] font-heading text-red-600 uppercase mb-1">{{ t('dashboard.mostRecent') }}</p>
    ...
    <div v-motion class="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 mb-8 sm:mb-10" style="animation-delay:0.08s">
      <!-- ASEAN Today Feed -->
      <BaseCard glass padding="lg">
        <h3 class="font-heading text-base font-bold text-ink dark:text-white mb-5 flex items-center gap-2">
          <PhNewspaper :size="16" weight="fill" class="inline text-teal-500" /> {{ t('dashboard.aseanToday') }}
        </h3>
        <div class="space-y-3">
      ...
      <BaseCard glass padding="lg">
        <h3 class="font-heading text-base font-bold text-ink dark:text-white mb-5 flex items-center gap-2">
          <PhTrophy :size="16" weight="fill" class="inline text-amber-500" /> {{ t('dashboard.leaderboard') }}
        </h3>
    ...
    <div v-motion class="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6" style="animation-delay:0.1s">
      <BaseCard glass padding="lg" id="responders">
        <h3 class="font-heading text-base font-bold text-ink dark:text-white mb-5">{{ t('dashboard.respondersDeployed') }} <PhShieldCheck :size="16" weight="fill" class="inline text-teal-500" /></h3>
      ...
      <BaseCard glass padding="lg">
        <h3 class="font-heading text-base font-bold text-ink dark:text-white mb-5">{{ t('dashboard.recentDisasters') }} <PhWarning :size="16" weight="fill" class="inline text-amber-500" /></h3>
        <!-- Loading state -->
        <div v-if="feedLoading" class="flex justify-center p-6">
          <div class="w-6 h-6 border-2 border-teal-500 border-t-transparent rounded-full animate-spin"></div>
        </div>
        
        <!-- Error state -->
        <div v-else-if="feedError && recentAlerts.length === 0" class="text-xs text-red-500 p-3 text-center bg-red-50 dark:bg-red-900/20 rounded-xl mb-2">
          {{ feedError }}
        </div>

        <div v-show="!feedLoading" class="space-y-2.5">
          <div v-for="r in recentAlerts" :key="r.id || r.country" class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
            <img :src="r.flag" class="w-6 h-auto inline-block rounded-sm shadow-sm" alt="Flag" />
            <div class="flex-1 min-w-0">
              <p class="text-xs font-medium font-body truncate text-ink dark:text-white" :title="r.recentDisaster">{{ r.recentDisaster }}</p>
          ...
          <div v-if="recentAlerts.length === 0" class="text-center text-xs text-gray-400 py-6">
            No recent major alerts in the ASEAN region.
          </div>
        </div>
      </BaseCard>
    </div>
...
import { useLiveDisasters } from '../composables/useLiveDisasters'
...
const readinessLabel = computed(() => {
  if (readinessScore.value >= 80) return t('dashboard.labels.fieldReady')
  if (readinessScore.value >= 60) return t('dashboard.labels.advanced')
  if (readinessScore.value >= 30) return t('dashboard.labels.developing')
  return t('dashboard.labels.beginner')
})

const topMetrics = computed(() => [
  { label: t('dashboard.metrics.activeResponders'), value: store.activeResponders.toLocaleString(), sub: t('dashboard.metrics.activeRespondersSub'), color: 'text-teal-600 dark:text-teal-400' },
  { label: t('dashboard.metrics.communityFund'), value: store.totalDonated.toLocaleString() + ' RC', sub: t('dashboard.metrics.communityFundSub'), color: 'text-orange-600 dark:text-orange-400' },
])

const { liveDisasters, loading: feedLoading, error: feedError } = useLiveDisasters()

const recentAlerts = computed(() => {
  if (liveDisasters.value.length > 0) return liveDisasters.value
  return [...store.regionData].sort((a, b) => b.disasterRiskIndex - a.disasterRiskIndex).slice(0, 6)
})
...
const tickerAlerts = computed(() => {
  const source = liveDisasters.value.length > 0 ? liveDisasters.value : store.regionData
  return source.map(r => ({
    flag: r.flag,
...
const todayFacts = computed(() => [
  { icon: PhLightbulb, title: t('dashboard.facts.didYouKnow'), text: t('dashboard.facts.didYouKnowDesc'), type: 'fact' },
  { icon: PhCrosshair, title: t('dashboard.facts.trainingTip'), text: t('dashboard.facts.trainingTipDesc'), type: 'tip' },
  { icon: PhChartBar, title: t('dashboard.facts.aseanStats'), text: t('dashboard.facts.aseanStatsDesc'), type: 'stat' },
])
...
const dashboardTourSteps = computed(() => [
  { title: t('dashboard.tour.missionControl'), description: t('dashboard.tour.missionControlDesc'), target: '#top-metrics', icon: PhCompass },
  { title: t('dashboard.tour.aseanIntel'), description: t('dashboard.tour.aseanIntelDesc'), target: '.country-card', icon: PhMapTrifold },
  { title: t('dashboard.tour.responderMap'), description: t('dashboard.tour.responderMapDesc'), target: '#responders', icon: PhUsersThree },
])
</script>
